...
def main():
    ...
    eagle3_model = DDP(
        eagle3_model,
        device_ids=[0],
        output_device=0,  # Recommended for clarity
        process_group=get_dp_group(),
    )
    ...
    with autocast(dtype=torch.bfloat16):
        ...
        for epoch in range(args.num_epochs):
            ...
            if epoch % args.save_interval == 0:
                ...
                #     dist.barrier()
                if dist.get_rank() == 0:
                    # 1. Get state dicts ONLY on rank 0 for efficiency.
                    #    Access the underlying model with .module to get the clean state dict.
                    model_state_dict = eagle3_model.module.state_dict()
                    optimizer_state_dict = optimizer.state_dict()

                    # 2. Prepare the training state to save (optimizer, epoch, etc.)
                    state_to_save = {
                        "epoch": epoch,
                        "args": args,
                    }
                    ...
                    # 3. Filter for the draft model's state dict (this logic remains the same)
                    draft_model_state_dict = {
                        k.replace("draft_model.", ""): v
                        for k, v in model_state_dict.items()
                        if "draft_model." in k and "embed" not in k.lower()
                    ...
                    # 4. Save the training state checkpoint
                    torch.save(
                        state_to_save,
                        os.path.join(epoch_output_dir, "training_state.pt"),
                    )
                    ...
                    # 5. Save the draft model using its own save method.
                    #    It's best to call this on the unwrapped model module.
                    #    This assumes `draft_model` is a submodule of your main model.
                    draft_model_to_save = eagle3_model.module.draft_model
